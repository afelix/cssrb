...
exports.rebase = function(src, config, move, copy) {
    ...
    for (var i = 0; i < uris.length; i++) {
        ...
        if (outPath) {
            setPath(uri, outPath);
            if (copy || move) {
                UTIL.mkdirs(PATH.dirname(config.toBase + outPath))
            }
...
function rename(path, patterns) {
    var e;
    /* prevent error if path contains params, like name.eot?#iefix */
    rename.pathParamsRE = rename.pathParamsRE  || new RegExp('[?#]');

    for (var i = 0; i < patterns.length; i++) {
        e = patterns[i];
        if (!rename.pathParamsRE.test(path) && e.re.test(path)) {
